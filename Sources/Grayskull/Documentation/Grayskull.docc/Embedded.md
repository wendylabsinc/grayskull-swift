...
WASI typically runs in single-threaded environments, so thread synchronization is unnecessary and adds overhead. The `Mutex` wrapper only exists when locking primitives are available.
...
| Windows | ✅ Yes | Win32 critical section |
